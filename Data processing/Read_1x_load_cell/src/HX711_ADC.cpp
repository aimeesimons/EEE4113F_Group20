...
float HX711_ADC::getData_MA() // return fresh data from the moving average dataset
{
	long data = 0;
	long lastMovingAverage = 0;
	lastMovingAverage = movingAverageData() ;
	data = lastMovingAverage - tareOffset ;
	float x = (float)data * calFactorRecip;
	return x;
}

float HX711_ADC::getData_Median() // return fresh data from the moving average dataset
{
	long data = 0;
	long lastMedianData = 0;
	lastMedianData = median_Data();
	data = lastMedianData - tareOffset ;
	float x = (float)data * calFactorRecip;
	return x;
}

float HX711_ADC::getData_Exp() // return fresh data from the moving average dataset
{
	long data = 0;
	long LastExp = 0;
	LastExp = exponentialAverageData();
	data = LastExp - tareOffset ;
	float x = (float)data * calFactorRecip;
	return x;
}
...
long int HX711_ADC :: getSmoothedData() {
    return smoothedData();
}

long HX711_ADC::movingAverageData() {

  int window_size = samplesInUse/4;
  long sum = 0;
  int index = 0;

  //Calculate the inital sum
  for (int i = 0; i < window_size; i++) {
    sum += dataSampleSet[index];
    index++;
  }

  long MAValue = sum / window_size;

  //Loop througb remaining data
  for (int i = window_size; i < samplesInUse; i++) {
    sum -= dataSampleSet[i - window_size]; // Remove oldest value from samples
    sum += dataSampleSet[index];      // Add newest value from samples
    index++;
    MAValue = sum / window_size;    // Calculate average
  }

  return MAValue;
}

long HX711_ADC::exponentialAverageData() {

    //alpha value
    const float alpha = 0.7;

    //First samples
    long EMAVlaue = dataSampleSet[0]+dataSampleSet[1]+dataSampleSet[3];

    // Loop through other samples updating the EMAValue
    for (int i = 3; i < samplesInUse; i++) {
        // Update the smoothed value
        EMAVlaue = alpha * dataSampleSet[i] + (1 - alpha) * EMAVlaue;
    }

    return EMAVlaue;
}

long HX711_ADC::median_Data()
{
    int window_size = 15; // Must be an odd number

    long temp_window[window_size];

    //Loop through all the values in the sample array
    for (int i = 0; i < samplesInUse; i++) {
        //Create a new array with the samples in the window
        for (int j = 0; j < window_size; j++) {
            int index = i + j - window_size / 2;  // Adjust for centered window
            if (index < 0) {
                index += samplesInUse;  // Handle wrap-around for negative indexes
            } else if (index >= samplesInUse) {
                index -= samplesInUse;
            }
            temp_window[j] = dataSampleSet[index];
        }

        // Sort the samples in the window using bubble sort
        for (int j = 0; j < window_size - 1; j++) {
            for (int k = j + 1; k < window_size; k++) {
                if (temp_window[j] > window[k]) {
                    long temp = temp_window[j];
                    temp_window[j] = temp_window[k];
                    temp_window[k] = temp;
                }
            }
        }
        ...
        long median = temp_window[window_size / 2];
        return median;
    }
}
